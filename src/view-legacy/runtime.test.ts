...
describe("View", () => {
    ...
    describe("JSX-compatible createElement function", () => {
        ...
        it("should create a component function", async (done) => {
            let _html: any
            const Component = (props: any) => h("span", null, props.text)
            const Root = () => {
                // h is only available within a render root
                const elem = h(Component, { text: "Hello" })
                expect(elem).toBeInstanceOf(Node)
                // grab the html for testing while it's still a node
                _html = getHTML(elem)
                return elem
            }

            const remove = render(Root, document.body)
            expect(_html).toBe("<span>Hello</span>")
